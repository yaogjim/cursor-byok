...
<template>
  <div class="flex h-full min-h-0 flex-col gap-4 overflow-y-auto scroll-shadow-bottom p-4 pt-0 text-[var(--color-text)]">
    <div class="flex flex-wrap items-start justify-between gap-3">
      <div>
        ...
        <div class="text-sm text-[var(--color-text-secondary)]">
          影响本机客户端行为，不包含 Cursor/Gateway 运行配置
        </div>
      </div>
      <div class="center-row gap-2">
        <span
          class="text-xs"
          :class="configSectionDirty.settings ? 'text-amber-700 dark:text-amber-300' : 'text-[var(--color-text-muted)]'"
        >
    ...
    <div class="text-xs leading-relaxed text-[var(--color-text-muted)]">
      本页保存只包含日志、外观和客户端行为设置，不包含 Cursor 运行模式、Gateway 或上游模型。
    </div>
  </div>
</template>