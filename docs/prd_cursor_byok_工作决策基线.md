...
3. **隐私边界不能被功能便利绕过**：调试、审计和错误处理不能以落盘正文、凭据或完整请求为代价。
...
### 5.1 原始 observability

- 默认关闭。
- 不读取真实会话正文，不保存 TLS 明文抓包。
- 不恢复默认写入完整 Bidi、provider request/response、Prompt、源码、diff 或历史正文。

### 5.2 专用隐私审计

专用观察器默认关闭，只允许覆盖明确标记的 17 条 relay RPC 和 provider 观察点。允许记录的内容仅包括：
...
- 请求进入未选择的新外部目标。
- 真实正文、源码、Prompt、凭据或完整 headers 被写入文件、日志或 artifact。
