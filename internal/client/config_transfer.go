...
// importedUserConfigDocument 内联 noad 的 serverconfig.Config 作为唯一 schema 事实源。
// 注意：noad 的 Config 已声明 `routing`（RoutingConfig/ mode），此处不得再声明
// LegacyRouting 之类的同 key 字段，否则 yaml.v3 会因 duplicated key 'routing' panic。
