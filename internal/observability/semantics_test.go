...
func TestNormalizeEventSemanticsProjectsSeverity(t *testing.T) {
	tests := []struct {
		...
		{name: "retrying attempt after server 5xx stays warning", event: Event{Status: "retrying", ErrorCategory: "server_5xx", SemanticOutcome: OutcomeDegraded, Fields: map[string]any{"retry_decision": "retry"}}, severity: SeverityWarning},
		{name: "retrying stream pre-event eof stays warning", event: Event{Status: "retrying", ErrorCategory: "stream_decode", SemanticOutcome: OutcomeDegraded, Fields: map[string]any{"retry_decision": "retry_stream_pre_event_eof"}}, severity: SeverityWarning},
		{name: "final exhausted server 5xx stays error", event: Event{Status: "error", ErrorCategory: "server_5xx", SemanticOutcome: OutcomeFailed, Fields: map[string]any{"retry_decision": "exhausted"}}, severity: SeverityError},
		{name: "no retry status stays error", event: Event{Status: "error", ErrorCategory: "server_5xx", SemanticOutcome: OutcomeFailed, Fields: map[string]any{"retry_decision": "no_retry_status"}}, severity: SeverityError},
		{name: "canceled attempt stays info", event: Event{Status: "canceled", ErrorCategory: "context_canceled", SemanticOutcome: OutcomeCanceled, Fields: map[string]any{"retry_decision": "no_retry_context"}}, severity: SeverityInfo},
		{name: "retrying without explicit decision is not downgraded", event: Event{Status: "retrying", ErrorCategory: "server_5xx", SemanticOutcome: OutcomeDegraded}, severity: SeverityError},
