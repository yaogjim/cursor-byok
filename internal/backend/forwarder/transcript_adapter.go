...
func normalizeAgentTranscriptsFolder(path string) string {
	...
	return normalizeAgentTranscriptsFolderWithin(path, filepath.Join(home, ".cursor", "projects"))
}

func normalizeAgentTranscriptsFolderWithin(path string, projectsRoot string) string {
	trimmed := strings.TrimSpace(path)
	root := filepath.Clean(strings.TrimSpace(projectsRoot))
	if trimmed == "" || !filepath.IsAbs(trimmed) || projectsRoot == "" || !filepath.IsAbs(root) {
		return ""
	}
	...
	if projectPath := filepath.Dir(relative); projectPath == "." || projectPath == string(filepath.Separator) {
		return ""
	}
	return cleaned
}
