...
func (host *Host) rebuildLocked(cfg serverconfig.Config) error {
	...
	host.mux = server.New(
		...
		server.FallbackUpstream(func(ctx *server.Context) error {
			if ctx == nil {
				return fmt.Errorf("server context is nil")
			}
			return upstream.DirectAction(routeDeps, upstream.CompatRouteConfig{Name: ctx.RouteName})(ctx)
		}),
		...
		cursorControlPlaneProcedureWithFallback(
...
func tabServerProcedure(pattern string, name string, protocol server.RouteOption, deps upstream.Dependencies) server.Option {
	forward := upstream.ForwardAction(deps, upstream.CompatRouteConfig{Name: name, Audit: true})
...
func cursorControlPlaneProcedure(
	...
	return cursorControlPlaneProcedureWithFallback(pattern, name, protocol, authorizationProvider, deps, notFound)
}

func cursorControlPlaneProcedureWithFallback(
	pattern string,
	name string,
	protocol server.RouteOption,
	authorizationProvider upstream.AuthorizationProvider,
	...
	action := cursorControlPlaneAction(authorizationProvider, deps, name, fallback)
	return server.POST(pattern,
		server.Name(name),
		protocol,
		server.Local(action),
